...
public class SuperstructureSubsystem extends SubsystemBase {
    ...
    public void periodic() {
        if (fmsSubsystem.getMatchStatus() == MatchStatus.AUTON) {
            lightBarSubsystem.setLightBarStatus(LightBarStatus.AUTON, 0);
        } else if (fmsSubsystem.getMatchStatus() == MatchStatus.ENDGAME) {
            lightBarSubsystem.setLightBarStatus(LightBarStatus.ENDGAME, 0);
        } else {
            lightBarSubsystem.setLightBarStatus(LightBarStatus.DORMANT, 0);
        }
    }
}